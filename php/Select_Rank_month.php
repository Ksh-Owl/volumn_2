...
//$sql = "SELECT ID,user_ID,friend_ID,friend_name,'{$userID}' AS meID FROM friend_list WHERE user_ID ='{$userID}' OR friend_ID = '{$userID}' ;";
 ...
 WHERE fl.user_ID ='{$userID}' OR fl.friend_ID = '{$userID}'
  ...
  ( SELECT wr.userID,SUM(pr.Perform_count * pr.Perform_weight) AS volumn
    ...
    WHERE (set_list.DATE > LAST_DAY(NOW() - interval 1 month) AND set_list.DATE <= LAST_DAY(NOW()))
...
$sql_me  = " SELECT wr.userID,SUM(pr.Perform_count * pr.Perform_weight) AS volumn
FROM set_list
INNER JOIN workoutList_Record AS wr ON wr.workout_ID = set_list.workout_ID

INNER JOIN perform_Record AS pr ON set_list.ID = pr.set_ID

WHERE (set_list.DATE > LAST_DAY(NOW() - interval 1 month) AND set_list.DATE <= LAST_DAY(NOW())) AND userID = '{$userID}'



GROUP BY wr.userID ;";

$result_me = mysqli_query($con,$sql_me);  
...
while($row_me = mysqli_fetch_array($result_me))
{
    
    array_push($me,array('userID'=>$row_me["userID"],'volumn'=>$row_me["volumn"] ));
}


header('Content-Type: application/json; charset=utf8');
$json = json_encode(array("Rank"=>$response,"me"=>$me), JSON_PRETTY_PRINT+JSON_UNESCPED_UNICOCDE );
echo $json;
